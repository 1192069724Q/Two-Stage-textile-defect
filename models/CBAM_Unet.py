...
class CBAM_Unet(nn.Module):
    ...
    def __init__(
        ...
        self.mc=CBAM(in_channel=ch)
        self.a1=CBAM(in_channel=32)
        self.a2 = CBAM(in_channel=32)
        self.a3 = CBAM(in_channel=64)
        self.a4 = CBAM(in_channel=64)
        self.a5 = CBAM(in_channel=128)
    ...
    def get_feature_vectors(self, x):
        ...
        """
        hs = []
        result = dict(down=[], up=[],down_1=[],up_1=[],up_2=[])
        h = x.type(self.inner_dtype)
        ...
        h = self.middle_block(h)
        result["middle_1"] = h.type(x.dtype)
        h = self.mc(h)
        result["middle_2"] = h.type(x.dtype)
        n=1
        for module in self.output_blocks:
            a=hs.pop()
            result["up_2"].append(a.type(x.dtype))
            if  n==2:
                a=self.a5(a)
                result["up_1"].append(a.type(x.dtype))
            if n==4:
                result["up_1"].append(a.type(x.dtype))
            if n==6:
                a=self.a3(a)
